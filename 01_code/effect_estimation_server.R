...
# parallel plan
future::plan(multisession, workers = base::floor(parallel::detectCores()*0.5))
...
data <- tibble::tibble(
  ...
  dplyr::filter(str_detect(path_in, 'correlated')) %>%
...
save(sim_results, file ='Z:/Data/baseline_try_correlated.RData')
