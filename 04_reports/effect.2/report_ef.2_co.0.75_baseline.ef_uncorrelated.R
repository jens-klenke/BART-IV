...
# load data
load(here::here('03_sim_results/effect.2/ef.2_co.0.75_baseline.ef_uncorrelated.RData'))
...
## Theoretical results ----
theoretical_df <- df %>%
  dplyr::filter(stringi::stri_detect(result_names, regex = 'theoretical_results')) %>%
...
## Frequentist IV results ----
Freq_results <- df %>%
  #  filter for main results
  dplyr::filter(stringi::stri_detect(result_names, regex = 'bcf_results')) %>%
  tidyr::unnest(results) %>%
  # get model to distingush between bayes and frequentist 
  dplyr::mutate(model = names(results)) %>%
  # filter for freq results
  dplyr::filter(model == 'bcfivResults') %>%
...
Freq_results %>%
  ...
  geom_text(data = get_counts(Freq_results), aes(x = Inf, y = Inf, label = paste("n =", n)),
            hjust = 1.1, vjust = 1.1, size = 3, color = '#004c93') +
  geom_vline(data = get_vline(Freq_results), aes(xintercept = x_line), color = '#004c93', linewidth = 0.75, linetype = 'longdash')
...
bayes_results <- df %>%
  ...
  dplyr::mutate(model = names(results)) %>%
  # filter for freq results
  dplyr::filter(model == 'bayes_ivResults') %>%
  # unnest bcf and bcfs results
  tidyr::unnest(results) %>%
  dplyr::arrange(ncov, ID) %>%
  dplyr::mutate(subgroup = dplyr::case_when(
...
bayes_results %<>%
  ...
  # force double datatype
  dplyr::mutate(CCACE = as.double(CCACE),
                `CCACE l-95% CI` = as.double(`CCACE l-95% CI`),
                `CCACE u-95% CI` = as.double(`CCACE u-95% CI`),
                Pi_obs = as.double(Pi_obs),
                ITT = as.double(ITT),
                Pi_compliers = as.double(Pi_compliers)
  )
...
bayes_results %>%
  ...
  ggplot2::theme_bw() +
  # Add number of observations as text
  geom_text(data = get_counts(bayes_results), aes(x = Inf, y = Inf, label = paste("n =", n)),
            hjust = 1.1, vjust = 1.1, size = 3, color = '#004c93') +
  geom_vline(data = get_vline(bayes_results), aes(xintercept = x_line), color = '#004c93', linewidth = 0.75, linetype = 'longdash')
